...
VERB_SYNONYMS = ["go", "look", "take", "drop", "use", "unlock"]
...
def parse_command(user_input: str) -> Optional[Command]:
    ...
    if len(parts) == 1 and parts[0] in DIRECTION_WORDS:
        return Command(verb="go", target=DIRECTION_WORDS[parts[0]], raw=user_input)
    ...
    if verb == "go":
        if len(parts) > 1 and parts[1] in DIRECTION_WORDS:
            return Command(verb="go", target=DIRECTION_WORDS[parts[1]], raw=user_input)
